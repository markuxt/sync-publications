 ...
 */
export async function extractPageText(pdfBuffer) {
 ...
 * failed).
 */
export function locateAbstractPage(abstract, pageTexts) {
    if (!abstract)
        return null;
    if (!pageTexts.length)
        return null;
 ...
 */
let pdftoppmCache = null;
export async function hasPdftoppm() {
    if (pdftoppmCache !== null)
 ...
 */
export async function renderPageWithPdftoppm(pdfPath, pageNumber, outPath, dpi = SCREENSHOT_DPI) {
 ...
 */
export function _expectedMinShortSide(dpi = SCREENSHOT_DPI) {
 ...
 */
export async function processPdf(work, outDir, relativeDir) {
    if (!work.pdfUrl) {
